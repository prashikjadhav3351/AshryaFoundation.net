...
<body>
    ...
    <script>
        $(document).ready(function() {
            $('#change-password-container').on('submit', function(e) {
                e.preventDefault();
                
                $.ajax({
                    type: 'POST',
                    url: 'changepw.php', // Make sure this URL points to your PHP script
                    data: $(this).serialize(),
                    dataType: 'json',
                    success: function(response) {
                        $('#message').text(response.message).fadeIn().delay(3000).fadeOut();
                    },
                    error: function() {
                        $('#message').text('An error occurred. Please try again.').fadeIn().delay(3000).fadeOut();
                    }
                });
            });
        });
